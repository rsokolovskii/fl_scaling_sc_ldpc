...
// April 2019
//
// Limited iterations, streaming, doping - Roman Sokolovskii
///////////////////////////////////////////////////////////////////////////
...
#define Def_M                      2500    // lifting factor
...
#define Def_epsIni				   0.46
#define Def_epsDelta			   0.005

//MaxNumIterTurboDecoding
#define Def_MaxNumIt			   500
//Number Of Simulated Points
#define Def_NUM_POINTS		       1//51
//MinimumNumberOfFramesInError
#define Def_MinNumberFramesError   500
#define Def_MinNumberFramesSim     3000
#define Def_MaxNumberFramesSim     500
//CheckFrames
...
void risultati(int sim, int n, int L, int W, double epsilon, double rate, int index)
{
    ...
    //sprintf(stringa,"/Users/graell/Alex/Projects/CSA/Results/BERFER_PoissonLDPC_n%d_dv%d_%f.txt",n,dv,rate);
    ...
    if(sim==0)
    {
        //fout  =fopen("/Users/graell/Alex/Projects/CSA/Results/BERFER_n%d_dv%d.txt","w",n,dv);
    ...
    //fout  =fopen("/Users/graell/Alex/Projects/CSA/Results/BERFER_n%d_dv%d.txt","a",n,dv);
...
int decodeBP(int n, int nk, int L, int W, int VNsPos, int CNsPos,
                int *num_blocks_err, int *num_erasures_exp, int *num_blocks_err_exp, FILE* fout, int is_term)
{
    // Lji : information that VN j sends to CN i
    // Lij : information that CN i sends to VN j
    ...
    *num_blocks_err_exp = 0;
    ...
    do
    {
        ...
        if (is_term)
            ...
            cn_lim = Def_L * Def_CNsPos;  // truncated -- no CNs after the L-th position
        ...
        fprintf(fout, "%d\t%d\t", iter, deg_1_iter);
        ...
        fprintf(fout, "%d\t%d\n", NumErasuresPrec - NumErasures, first_erased / VNsPos);
...
{ // loop on simulation points
    ...
    char fout_name[500];
    if (is_term)
        sprintf(fout_name,"trajectories_%.4f_terminated_SC_LDPC_%d_%d_L%d_M%d_BP_Full_%dit_Random_BLER_%d.dat",epsilon, Def_dv,Def_dc,Def_L,Def_M,MaxNumIt,index);
    else
        sprintf(fout_name,"trajectories_%.4f_truncated_SC_LDPC_%d_%d_L%d_M%d_BP_Full_%dit_Random_BLER_%d.dat",epsilon, Def_dv,Def_dc,Def_L,Def_M,MaxNumIt,index);
		...
		{ // loop on simulated frames
          ...
          // if(numSS==0)
            ...
            NumErasures = decodeBP(n,nk,L,W,VNsPos,CNsPos,
                    &num_blocks_err, &num_erasures_exp, &num_blocks_err_exp, fout, is_term);
